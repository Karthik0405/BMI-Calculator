...
const BmiCalculator = () => {
  const storrdHeight = JSON.parse(localStorage.getItem('height'))
  const storrdWeight = JSON.parse(localStorage.getItem('weight'))
  const [height, setHeight] = useState(
    storrdHeight === null ? 170 : storrdHeight,
  )
  const [weight, setWeight] = useState(
    storrdWeight === null ? 60 : storrdWeight,
  )
